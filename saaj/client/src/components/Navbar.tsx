...
import defaultLogo from "@assets/logo_1773491639025.png";
...
export default function Navbar() {
  ...
  const logoSrc = settings?.site_logo || defaultLogo;
                  ...
                  <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded shadow-lg z-[200] min-w-[180px] py-1">
                    {CURRENCIES.map((c) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-[11px] tracking-wide hover:bg-gray-50 flex items-center justify-between transition-colors ${
                          selectedCurrency.code === c.code ? "text-[#c4151c] font-semibold" : "text-gray-700"
                        }`}
                      >
                        <span>{c.label}</span>
              ...
              {navLinks.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-5 py-3.5 border-b border-gray-50 text-xs tracking-[0.1em] uppercase font-medium transition-colors ${
                      active
